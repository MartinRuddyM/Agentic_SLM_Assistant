...
from manage import EmbeddingDB
...
def main():
    ...
    default_chat = ChatGoogleGenerativeAI(
    ...
    while True:
        ...
        answer, reasoning = ReAct_process(query, react_task_desc, system_prompts, default_chat, cheap_chat)
        ...
        conversation.add_interaction(query, final_answer, reasoning)
    ...
    if len(conversation.history) > 0:
        ...
        if len(existing_user_info) > 0:
            new_user_info = contrast_user_information(existing_user_info, new_user_info, cheap_chat, system_prompts)
